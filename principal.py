...
def test_agregar_comanda_valida():
    ...
    hilo = threading.Thread(target=escribir_base_de_datos, args=(threading.current_thread().name, num_comanda, barbacoa, quesadilla, bebida, consome), name=f'HiloAgregarComanda')
    hilo.start()
    hilo.join()
